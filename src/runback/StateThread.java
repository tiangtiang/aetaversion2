...
public class StateThread implements Runnable {
	...
	public boolean terminalInsertIntoDB(Map<String, String> kvs) {
		...
		// 获取终端的系统时间
		long TerminalSysTime = Long.parseLong(kvs.get("CurrentTime"
				.toLowerCase())) * 1000;
		Date time_tmp = new Date(TerminalSysTime);
		String TerminalTime = format.format(time_tmp);
		// 获取终端上次重启时的重启时间
		long TerminaRebootLastTime = Long.parseLong(kvs.get("RunUpTime"
				.toLowerCase())) * 1000;
		time_tmp = new Date(TerminaRebootLastTime);
		String TerminaRebootLastTimeString = format.format(time_tmp);
		// 终端运行的软件版本信息
		String TerStartTime = kvs.get("Version".toLowerCase());
		// SDStatus-SD卡状态
		int SDStatus = Integer.parseInt(kvs.get("SDState".toLowerCase()));
		// FreeSpaceRate-SD卡剩余空间
		float FreeSpaceRate = Float.parseFloat(kvs.get("FreeSpaceRate"
				.toLowerCase()));
		// Network-网络状况
		int Network = Integer.parseInt(kvs.get("Network".toLowerCase()));
		// DataRate-网口数据率
		...
		try {
			...
			prestmt = conn
					.prepareStatement("INSERT into "
							...
							+ "values(" + deviceId + ",'"
							+ CurrentSysTimeString + "','" + TerminalTime
							+ "','" + TerminaRebootLastTimeString + "','"
			...
			if (flag1 > 0) {
				...
				if (ret.next()) {// 如果存在，则进行update操作
					prestmt = conn.prepareStatement("update "
							...
							+ CurrentSysTimeString + "',TerminalSysTime='"
							+ TerminalTime + "',TerStartTime='"
							+ TerminaRebootLastTimeString
							+ "',TerSoftVersion='" + TerStartTime
							+ "',SDStatus=" + SDStatus + ",FreeSpace="
							+ FreeSpaceRate + ",NetworkStatus=" + Network
	...
	public boolean ProbeInsertIntoDB(Map<String, String> kvs) {
		...
		String TerminalTime = format.format(time_tmp);
		// 终端运行的软件版本信息
		String Version = kvs.get("Version".toLowerCase());
		// EN-保留+使能端
		int EN = Integer.parseInt(kvs.get("EN".toLowerCase()));
		// SynInterval-同步间隔
		int SynInterval = Integer
				.parseInt(kvs.get("SynInterval".toLowerCase()));
		// ProbeID-探头号
		int ProbeID = Integer.parseInt(kvs.get("ProbeID".toLowerCase()));
		// ProbeType-探头类型
		int ProbeType = Integer.parseInt(kvs.get("ProbeType".toLowerCase()));
		// zerdrf_comp-零漂补偿系数
		int zerdrf_comp = Integer
				.parseInt(kvs.get("zerdrf_comp".toLowerCase()));
		// multiple-放大倍数
		int multiple = Integer.parseInt(kvs.get("multiple".toLowerCase()));
		try {
			PreparedStatement prestmt;
			// 开启事务
			conn.setAutoCommit(false);
			...
			prestmt = conn
					.prepareStatement("INSERT into "
							...
							+ "values(" + ProbeID + "," + ProbeType + ","
							+ terminalId + ",'" + CurrentSysTimeString + "','"
							+ TerminalTime + "','" + Version + "'," + EN + ","
							+ SynInterval + "," + zerdrf_comp + "," + multiple
							+ ")");
			int flag1 = prestmt.executeUpdate();
			if (flag1 > 0) {
				// 查看当前终端状态表是否存在此ProbeID的状态数据
				prestmt = conn
						.prepareStatement("select t.* from "
								+ tableName_current + " t where t.ProbeID = "
								+ ProbeID);
				ResultSet ret = prestmt.executeQuery();
				if (ret.next()) {// 如果存在，则进行update操作
					prestmt = conn.prepareStatement("update "
							+ tableName_current + " t set ProbeType="
							+ ProbeType + ",TerminalID=" + terminalId
							+ ",ServerSysTime='" + CurrentSysTimeString
							+ "',TerminalSysTime='" + TerminalTime
							+ "',ProSoftVersion='" + Version + "',DataEnable="
							+ EN + ",SyncInterVal=" + SynInterval
							+ ",ZeroCorrectValue =" + zerdrf_comp
							+ ",DataAmpTimes=" + multiple + " where t.ProbeID="
							+ ProbeID);
					int flag2 = prestmt.executeUpdate();
					if (flag2 > 0) {// 操作成功，提交事务
						conn.commit();
