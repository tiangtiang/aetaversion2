...
public class LogfileThread implements Runnable{
	...
	public void terminalInsertIntoDB(){
		...
		tableName = "terminallog";
			...
			prestmt = conn.prepareStatement("INSERT into "+tableName+"(Time,TerminalID,LogType,Length,Event,)"
						+"values("+Time+","+deviceId+","+logType+","+logLength+",'"+logContent+"')");
	...
	public void ProbeInsertIntoDB(){
		...
		tableName = "probelog";
			...
			PreparedStatement prestmt;
			//插入终端状态数据到terminallog表中
			prestmt = conn.prepareStatement("INSERT into "+tableName+"(Time,ProbeID,TerminalID,LogType,Length,Event,)"
						+"values("+Time+","+deviceId+","+terminalId+","+logType+","+logLength+",'"+logContent+"')");
			prestmt.executeUpdate();
			prestmt.close();
			conn.close();
			} catch (SQLException e) {
