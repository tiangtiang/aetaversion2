...
public class DatapostThread implements Runnable{
	...
	public void rawAndFeatureDataIntoDB(int dataType){
		...
		//特征值-------振铃计数
		int  RDC = getRDC(0);//这里振铃计数的阈值暂时设为0.
		Peak peak = this.getPFandPA(dataType);
		//特征值-------全局峰值频率
		float peak_frequency = peak.getPeak_frequency();
		//特征值-------全局峰值幅值
		...
		//插入原始数据
		prestmt = conn.prepareStatement("insert into "+tableNames[1]+"(Time,ProbeID,Length,Value) values(?,?,?,?)");
		...
		if(flag1 >0 ){
			//插入特征数据
			prestmt = conn.prepareStatement("insert into "+tableNames[0]+"(Time,ProbeID,average,Ring_down_count,peak_frequency,peak_amplitude) values(?,?,?,?)");
			prestmt.setInt(1, Time);
			prestmt.setInt(2, ProbeID);
			prestmt.setFloat(3, average);
			prestmt.setInt(4, RDC);
			prestmt.setFloat(5, peak_frequency);
	...
	public void tempretureDataIntoDB(int dataType){
		...
		//插入原始数据
		try {
			prestmt = conn.prepareStatement("insert into "+tableNames[0]+"(Time,TerminalID,ProbeID,Tempreture) values(?,?,?,?)");
			prestmt.setInt(1, Time);
	...
	public int getRDC(int threshold){
		int result = 0;
		//temp 为前一个数
		short temp = dataAfterAnalysis[0];
		//这里是否需要对数组长度做检查还是在数据接收时就对数组长度作检查？？？？？？？
		for(int i = 1; i<dataAfterAnalysis.length;i = i++){
			//temp1为后一个数
...
//	    		ft->FreqAmplitude = max/m;
	...
	//临时存储fft的结果的类
	 class Peak{
		 ...
		 public int peak_frequency;		//全局峰值频率
		...
		public int getPeak_frequency() {
			return peak_frequency;
		}
		public void setPeak_frequency(int peakFrequency) {
...
//	    ft->FreqAmplitude = ft->FreqAmplitude*2;
	 ...
	 public Peak getPFandPA(int dataType){
		 Peak peak = null;
		 switch(dataType){
		 case 1: peak = this.getFeature_EM_Low(this.dataAfterAnalysis);
		 		 break;
		 case 2: peak = this.getFeature_EM_Full(this.dataAfterAnalysis);
 		 		 break;
		 case 3: peak = this.getFeature_SD_Low(this.dataAfterAnalysis);
		 		 break;
		 case 4: peak = this.getFeature_SD_Full(this.dataAfterAnalysis);
	 		     break;
	 	 default:System.out.println("dataType of rawData is wrong! The value of dataType is "+dataType);
		 }
		 return peak;
	 }
